...
touch $ROOT_CA/db/fabio-project-ca.db
touch $ROOT_CA/db/fabio-project-ca.db.attr
...
touch $ROOT_CA_INTERMEDIATE/db/fabio-project-ca.db
touch $ROOT_CA_INTERMEDIATE/db/fabio-project-ca.db.attr
...
#  Create the root key
#Encrypt the root key with AES 256-bit encryption and a strong password.
#openssl genrsa -aes256  -out $ROOT_CA_INTERMEDIATE/private/fabio-project.key.pem 2048
openssl genrsa \
  -out $ROOT_CA_INTERMEDIATE/private/fabio-project.key.pem \
  2048
# chmod 400 $ROOT_CA_INTERMEDIATE/private/fabio-project.key.pem


openssl req -config $CONF_DIR_INTERMEDIATE/openssl.cnf \
  -key $ROOT_CA_INTERMEDIATE/private/fabio-project.key.pem \
  -new -sha256 -out $ROOT_CA_INTERMEDIATE/csr/fabio-project.csr.pem

#Create  server a certificate
# sign the CSR
openssl ca -config $CONF_DIR_INTERMEDIATE/openssl.cnf -batch \
  -extensions server_cert -days 375 -notext -md sha256 \
  -in $ROOT_CA_INTERMEDIATE/csr/fabio-project.csr.pem \
  -out $ROOT_CA_INTERMEDIATE/certs/server.fabio-project.cert.pem
# chmod 444 $ROOT_CA_INTERMEDIATE/certs/server.fabio-project.cert.pem

openssl x509 -noout -text \
  -in $ROOT_CA_INTERMEDIATE/certs/server.fabio-project.cert.pem
...
openssl ca -config $CONF_DIR_INTERMEDIATE/openssl.cnf -batch \
  ...
  -in $ROOT_CA_INTERMEDIATE/csr/fabio-project.csr.pem  \
  -out $ROOT_CA_INTERMEDIATE/certs/client.fabio-project.cert.pem
# chmod 444 $ROOT_CA_INTERMEDIATE/certs/client.fabio-project.cert.pem
...
# create fullchain
cat $ROOT_CA_INTERMEDIATE/certs/server.fabio-project.cert.pem \
...
cp -r $ROOT_CA/certs/ca.cert.pem $ROOT_DIR/ca/fabio-project-root-ca.crt.pem
cp -r $ROOT_CA/private/ca.key.pem $ROOT_DIR/ca/fabio-project-root-ca.key.pem
...
cp -r $ROOT_CA_INTERMEDIATE/private/fabio-project.key.pem $ROOT_DIR/server/privkey.pem
cp -r $ROOT_CA_INTERMEDIATE/certs/server.fabio-project.cert.pem $ROOT_DIR/server/cert.pem
...
openssl verify -CAfile $ROOT_CA_INTERMEDIATE/certs/ca-chain.cert.pem \
  $ROOT_CA_INTERMEDIATE/certs/server.fabio-project.cert.pem

# verify server
openssl verify -CAfile $ROOT_CA_INTERMEDIATE/certs/ca-chain.cert.pem \
  $ROOT_CA_INTERMEDIATE/certs/client.fabio-project.cert.pem

openssl rsa -noout -modulus -in $ROOT_DIR/server/privkey.pem
openssl req -noout -modulus -in $ROOT_CA_INTERMEDIATE/csr/fabio-project.csr.pem
